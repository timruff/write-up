...
child_process:
	...
	# check GET or POST
	mov eax,buffer	      # get request string
	mov ebx,request_get   # get string GET for compare
	cmp eax,ebx	      # compare 4 first bytes
	je handle_get	      # if match go manage get
	mov ebx,request_post  # get string POST for compare
	cmp eax,ebx	      # compare 4 first bytes
	je handle_post	      # if match go manage post
	jmp program_exit      # else other go to exit
handle_get:
	# open()
	...
	mov rax,1	   # number syscall write
	syscall		   # call syscall
	jmp program_exit   # end program
handle_post:
	# open()
	lea rdi,[rsi+5]    # path of file and ignore GET
	movb [rdi+16],0	   # cut end of string
