...
**What is the flag?**
```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme1 
flag{not_that_kind_of_elf}
```

Pour le premier flag il juste exécuter le cackme1.  

## Task 2 Crackme2 ##
**What is the super secret password ?**
```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme2 
Usage: ./crackme2 password
tim@kali:~/Bureau/tryhackme/write-up$ radare2 ./crackme2 
[0x080483a0]> aaa
...
│      │    0x080484db      6874860408     push str.super_secret_password ; 0x8048674 ; "super_secret_password" ; const char *s2
│      │    0x080484e0      50             push eax                    ; const char *s1
...
│      ││   0x080484f0      688a860408     push str.Access_denied.     ; 0x804868a ; "Access denied." ; const char *s
...
│     │││   ; CODE XREF from main @ 0x80484eb
│     ││└─> 0x08048504      83ec0c         sub esp, 0xc
│     ││    0x08048507      6899860408     push str.Access_granted.    ; 0x8048699 ; "Access granted." ; const char *s
...
Ici on utilise radar2.  
L'option : aaa permet d'analyser tout le fichier.
L'option : afl liste les fonctions du fichier.
On selection la fonction main avec s main.  
On l'affiche avec pdf.
A l'adresse 0x080484db, il y une instruction qui met en mémoire la chaîne de caractère "super_secret_password".  
Qui après  l'adresse 0x080484e1 la compare avec strcmp.  

Réponse : super_secret_password  

**What is the flag ?**
```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme2 super_secret_password
Access granted.
flag{if_i_submit_this_flag_then_i_will_get_points}
```

Il suffit juste de taper le mot de passe trouvé.  

Réponse : flag{if_i_submit_this_flag_then_i_will_get_points}

## Task 3 Crackme3 ##
** What is the flag? **
```bash
tim@kali:~/Bureau/tryhackme/write-up$ radare2 crackme3 
[0x08048440]> aaa
[x] Analyze all flags starting with sym. and entry0 (aa)
[x] Analyze function calls (aac)
[x] Analyze len bytes of instructions for references (aar)
...
0x080483c0    1 6            sym.imp.strcmp
0x080483d0    1 6            sym.imp.fwrite
0x080483e0    1 6            sym.imp.malloc
0x080483f0    1 6            sym.imp.puts
0x08048410    1 6            sym.imp.strlen
0x08048430    1 6            sym.imp.fprintf
0x080484f4   10 220          main
0x08048400    1 6            loc.imp.__gmon_start__
0x080484d0    4 35           fcn.080484d0
0x08048e10    4 42           fcn.08048e10
0x080486b0   17 874          fcn.080486b0
0x080485d0   14 219          fcn.080485d0
0x08048e02    1 4            fcn.08048e02
0x0804837c    3 46           fcn.0804837c
0x08048e45    1 17           fcn.08048e45
0x08048470    6 85           fcn.08048470
[0x08048440]> s main
[0x080484f4]> pdf
            ; DATA XREF from entry0 @ 0x8048457
┌ 220: int main (char **argv, char **envp);
│           ; arg char **argv @ ebp+0x8
│           ; arg char **envp @ ebp+0xc
│           ; var char *format @ esp+0x4
│           ; var char *nitems @ esp+0x8
│           ; var FILE *stream @ esp+0xc
│           0x080484f4      55             push ebp
│           0x080484f5      89e5           mov ebp, esp
│           0x080484f7      57             push edi
│           0x080484f8      56             push esi
│           0x080484f9      83ec10         sub esp, 0x10
│           0x080484fc      8b450c         mov eax, dword [envp]
│           0x080484ff      837d0802       cmp dword [argv], 2
│       ┌─< 0x08048503      7420           je 0x8048525
│       │   0x08048505      8b08           mov ecx, dword [eax]
│       │   0x08048507      a128b00408     mov eax, dword [obj.stderr] ; [0x804b028:4]=0
│       │   0x0804850c      894c2408       mov dword [nitems], ecx
│       │   0x08048510      890424         mov dword [esp], eax        ; FILE *stream
│       │   0x08048513      c7442404688e.  mov dword [format], str.Usage:__s_PASSWORD_n ; [0x8048e68:4]=0x67617355 ; "Usage: %s PASSWORD\n" ; const char *format
│       │   0x0804851b      e810ffffff     call sym.imp.fprintf        ; int fprintf(FILE *stream, const char *format,   ...)
│      ┌──< 0x08048520      e99f000000     jmp 0x80485c4
│      ││   ; CODE XREF from main @ 0x8048503
│      │└─> 0x08048525      8b7804         mov edi, dword [eax + 4]
│      │    0x08048528      893c24         mov dword [esp], edi        ; const char *s
│      │    0x0804852b      e8e0feffff     call sym.imp.strlen         ; size_t strlen(const char *s)
│      │    0x08048530      01c0           add eax, eax
│      │    0x08048532      890424         mov dword [esp], eax        ; size_t size
│      │    0x08048535      e8a6feffff     call sym.imp.malloc         ;  void *malloc(size_t size)
│      │    0x0804853a      89c6           mov esi, eax
│      │    0x0804853c      85f6           test esi, esi
│      │┌─< 0x0804853e      7527           jne 0x8048567
│      ││   0x08048540      a128b00408     mov eax, dword [obj.stderr] ; [0x804b028:4]=0
│      ││   0x08048545      8944240c       mov dword [stream], eax     ; FILE *stream
│      ││   0x08048549      c74424080100.  mov dword [nitems], 1       ; size_t nitems
│      ││   0x08048551      c74424040e00.  mov dword [format], 0xe     ; [0xe:4]=-1 ; 14 ; size_t size
│      ││   0x08048559      c704247c8e04.  mov dword [esp], str.malloc_failed_n ; [0x8048e7c:4]=0x6c6c616d ; "malloc failed\n" ; const void *ptr
│      ││   0x08048560      e86bfeffff     call sym.imp.fwrite         ; size_t fwrite(const void *ptr, size_t size, size_t nitems, FILE *stream)
│     ┌───< 0x08048565      eb5d           jmp 0x80485c4
│     │││   ; CODE XREF from main @ 0x804853e
│     ││└─> 0x08048567      893c24         mov dword [esp], edi        ; const char *s
│     ││    0x0804856a      e8a1feffff     call sym.imp.strlen         ; size_t strlen(const char *s)
│     ││    0x0804856f      89442408       mov dword [nitems], eax     ; int32_t arg_50h
│     ││    0x08048573      89742404       mov dword [format], esi     ; int32_t arg_4ch
│     ││    0x08048577      893c24         mov dword [esp], edi        ; int32_t arg_48h
│     ││    0x0804857a      c744240c0000.  mov dword [stream], 0       ; int32_t arg_54h
│     ││    0x08048582      e829010000     call fcn.080486b0
│     ││    0x08048587      893424         mov dword [esp], esi        ; const char *s
│     ││    0x0804858a      e881feffff     call sym.imp.strlen         ; size_t strlen(const char *s)
│     ││    0x0804858f      83f840         cmp eax, 0x40               ; 64
│     ││┌─< 0x08048592      7524           jne 0x80485b8
│     │││   0x08048594      893424         mov dword [esp], esi        ; const char *s1
│     │││   0x08048597      c74424048b8e.  mov dword [format], str.ZjByX3kwdXJfNWVjMG5kX2xlNTVvbl91bmJhc2U2NF80bGxfN2gzXzdoMW5nNQ ; [0x8048e8b:4]=0x79426a5a ; "ZjByX3kwdXJfNWVjMG5kX2xlNTVvbl91bmJhc2U2NF80bGxfN2gzXzdoMW5nNQ==" ; const char *s2
│     │││   0x0804859f      e81cfeffff     call sym.imp.strcmp         ; int strcmp(const char *s1, const char *s2)
│     │││   0x080485a4      85c0           test eax, eax
│    ┌────< 0x080485a6      7510           jne 0x80485b8
│    ││││   0x080485a8      c70424d08e04.  mov dword [esp], str.Correct_password_ ; [0x8048ed0:4]=0x72726f43 ; "Correct password!" ; const char *s
│    ││││   0x080485af      e83cfeffff     call sym.imp.puts           ; int puts(const char *s)
│    ││││   0x080485b4      31c0           xor eax, eax
│   ┌─────< 0x080485b6      eb11           jmp 0x80485c9
│   │││││   ; CODE XREFS from main @ 0x8048592, 0x80485a6
│   │└──└─> 0x080485b8      c70424f08e04.  mov dword [esp], str.Come_on__even_my_aunt_Mildred_got_this_one_ ; [0x8048ef0:4]=0x656d6f43 ; "Come on, even my aunt Mildred got this one!" ; const char *s
│   │ ││    0x080485bf      e82cfeffff     call sym.imp.puts           ; int puts(const char *s)
│   │ ││    ; CODE XREFS from main @ 0x8048520, 0x8048565
│   │ └└──> 0x080485c4      b8ffffffff     mov eax, 0xffffffff         ; -1
│   │       ; CODE XREF from main @ 0x80485b6
│   └─────> 0x080485c9      83c410         add esp, 0x10
│           0x080485cc      5e             pop esi
│           0x080485cd      5f             pop edi
│           0x080485ce      5d             pop ebp
└           0x080485cf      c3             ret

```
Même chose que le crack ci-dessus mais il transforme le mot passe entré en base64.   
Puis le compare avec ZjByX3kwdXJfNWVjMG5kX2xlNTVvbl91bmJhc2U2NF80bGxfN2gzXzdoMW5nNQ==.  

```bash
tim@kali:~/Bureau/tryhackme/write-up$ echo -n "ZjByX3kwdXJfNWVjMG5kX2xlNTVvbl91bmJhc2U2NF80bGxfN2gzXzdoMW5nNQ==" | base64 -d
f0r_y0ur_5ec0nd_le55on_unbase64_4ll_7h3_7h1ng5
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme3 "f0r_y0ur_5ec0nd_le55on_unbase64_4ll_7h3_7h1ng5"
Correct password!
```

On déchiffre la chaîne mis en base64 et on le vérifie.

La réponse est : f0r_y0ur_5ec0nd_le55on_unbase64_4ll_7h3_7h1ng5  
...
On regarde la fonction main il y pas de mot de passe, mais il y a une fonction qui compare le mot de passe.  
La fonction est sym.compare_pwd.

```bash
[0x00400520]> pdf @sym.compare_pwd
...
On remarque trois chaînes qui sont chiffrés :  
-7b175614497b5d49
-547b175651474157
-4053 
La chaîne est continu dans la mémoire.  
On remarque plus loin une autre fonction sym.get_pwd.  

```bash
[0x00400520]> pdf @sym.get_pwd
            ; CALL XREF from sym.compare_pwd @ 0x4006c2
┌ 77: sym.get_pwd (int64_t arg1);
│           ; var int64_t var_18h @ rbp-0x18
│           ; var int64_t var_4h @ rbp-0x4
│           ; arg int64_t arg1 @ rdi
│           0x0040062d      55             push rbp
│           0x0040062e      4889e5         mov rbp, rsp
│           0x00400631      48897de8       mov qword [var_18h], rdi    ; arg1
│           0x00400635      c745fcffffff.  mov dword [var_4h], 0xffffffff ; -1
│       ┌─< 0x0040063c      eb22           jmp 0x400660
│      ┌──> 0x0040063e      8b45fc         mov eax, dword [var_4h]
│      ╎│   0x00400641      4863d0         movsxd rdx, eax
│      ╎│   0x00400644      488b45e8       mov rax, qword [var_18h]
│      ╎│   0x00400648      4801c2         add rdx, rax
│      ╎│   0x0040064b      8b45fc         mov eax, dword [var_4h]
│      ╎│   0x0040064e      4863c8         movsxd rcx, eax
│      ╎│   0x00400651      488b45e8       mov rax, qword [var_18h]
│      ╎│   0x00400655      4801c8         add rax, rcx
│      ╎│   0x00400658      0fb600         movzx eax, byte [rax]
│      ╎│   0x0040065b      83f024         xor eax, 0x24               ; 36
│      ╎│   0x0040065e      8802           mov byte [rdx], al
│      ╎│   ; CODE XREF from sym.get_pwd @ 0x40063c
│      ╎└─> 0x00400660      8345fc01       add dword [var_4h], 1
│      ╎    0x00400664      8b45fc         mov eax, dword [var_4h]
│      ╎    0x00400667      4863d0         movsxd rdx, eax
│      ╎    0x0040066a      488b45e8       mov rax, qword [var_18h]
│      ╎    0x0040066e      4801d0         add rax, rdx
│      ╎    0x00400671      0fb600         movzx eax, byte [rax]
│      ╎    0x00400674      84c0           test al, al
│      └──< 0x00400676      75c6           jne 0x40063e
│           0x00400678      5d             pop rbp
└           0x00400679      c3             ret
```

Dans cette fonction on remarque le paterne est une boucle qui parcoure la chaîne.  
A l'adresse 0x0040065b on remarque qu'il un xor fait avec la valeur 0x24 (héxadécimal).  
Donc on remarque que la chaîne est chiffrée avec un xor (ou exclusif).  

Faisons la conversion, un site comme : [xor](http://xor.pw/#) peut nous faciliter la tâche.  
4053 547b175651474157 7b175614497b5d49
2424 2424242424242424 2424242424242424 XOR
dw   p_3ruces         _3r0m_ym  
En lisant le chaîne à l'envers on a ceci : my_m0r3_secur3_pwd

```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme4 my_m0r3_secur3_pwd
password OK
```

On le vérifie.

La réponse est : my_m0r3_secur3_pwd  

## Task 5 Crackme5 ##
**What is the input ?**
```bash
tim@kali:~/Bureau/tryhackme/write-up$ radare2 -d crackme5 
Process with PID 2076 started...
= attach 2076 2076
...
On remarque  dans la fonction, il y une chaîne codée caractère par caractère : OfdlDSA|3tXb32~X3tX@sX`4tXtz  

```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme5
Enter your input:
OfdlDSA|3tXb32~X3tX@sX`4tXtz  
Good game
```

Cette chaîne est la bonne réponse. 
Réponse : OfdlDSA|3tXb32~X3tX@sX`4tXtz  

## Task 6 Crackme6 ##
**What is the password ?**

```bash
tim@kali:~/Bureau/tryhackme/write-up$ radare2 -d crackme6 
Process with PID 2173 started...
= attach 2173 2173
bin.baddr 0x00400000
Using 0x400000
asm.bits 64
...
Dans la fonction main on vois une fonctione sym.compare_pwd.

```bash
[0x7fb10a96f090]> pdf @sym.compare_pwd
...
Dans cette fonction, il y a une autre fonction sym.my_secure_test.  

```bash
[0x7fb10a96f090]> pdf @sym.my_secure_test
...
Dans cette fonction on vois que des caractères sont testés un par un.  
49 51 51 55 95 112 119 100
1  3  3  7  _  p   w   d
On a : 1337_pwd

```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme6 1337_pwd
password OK
```

La réponse est : 1337_pwd 

## Task 7 Crackme7 ##
**What is the flag ?**

```bash
tim@kali:~/Bureau/tryhackme/write-up$ radare2 -d crackme7 
Process with PID 2237 started...
= attach 2237 2237
...
Dans la fonction main on remarque une chaîne interéssante, Wow such h4x0r!, en regardant plus haut on remarque à l'adresse 0x08048665 une instruction qui compare à la valeure 0x7a69.  
Convertisont cette valeur en décimal 0x7a69 = 31337.  

```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme7 
Menu:
...
Grace à ce nombre on obtient le flag.  
La réponse est : flag{much_reversing_very_ida_wow} 

## Task 8 Crackme8 ##
**What is the flag ?**
```bash
tim@kali:~/Bureau/tryhackme/write-up$ radare2 -d crackme8 
Process with PID 2330 started...
= attach 2330 2330
bin.baddr 0x08048000
Using 0x8048000
asm.bits 32
...
On voit à l'addresse 0x080484e4 qu'il y a une comparaison avec la valeure : 0xcafef00d. 
Et au dessus il à l'adresse  0x080484dc, il y une fonction atoi, qui convertire une chaîne de caractères en int.
le nombre cafef00d converti en décimal est : 3405705229.

Il faut faire l'opération inverse.  

```c
tim@kali:~/Bureau/tryhackme/write-up$ cat convert.c 
#include <stdio.h>

int main()
{
	int number = 3405705229;
	char text[20];
...
```
```bash
im@kali:~/Bureau/tryhackme/write-up$ gcc convert.c -o convert
tim@kali:~/Bureau/tryhackme/write-up$ ./convert 

```

Le petit programme en C fait l'opération inverse, il converti un nombre décimal en chaîne de caractères.  

```bash
tim@kali:~/Bureau/tryhackme/write-up$ ./crackme8 -889262067
Access granted.
flag{at_least_this_cafe_wont_leak_your_credit_card_numbers}
```

Grace au nombre trouvé on a notre réponse.  
Réponse : flag{at_least_this_cafe_wont_leak_your_credit_card_numbers}  